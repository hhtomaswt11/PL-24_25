...
class CodeGenerator:
    ...
    def _generate_boolean(self, node):
        self.emit(f"PUSHI {1 if node.leaf.lower() == 'true' else 0}")
    ...
    def _generate_binary_op(self, node):
        ...
        self.emit(ops_map.get(op, f"#UNKNOWN_OP {op}"))

    def _generate_unary_op(self, node):
        self._generate_code(node.children[0])
        if node.leaf.lower() == 'not':
            self.emit("NOT")
